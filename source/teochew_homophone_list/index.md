...
|h| ① hun 芬 酚 吩 氛 分 紛{纷} 汾 葷{荤} 昏 婚 渾{浑} 燻{熏} 暈{晕} 薰   ② hún 粉 忿   ③ hùn 奮{奋} 憤{愤} 訓{训}   ④ hut 沸 拂 氟 弗 忽 彿{佛}`(彷~/phâng-~)`   ⑤ hûn 痕 魂 雲{云}`(~雨/~-hõu)`   ⑥ hũn 渾{浑} 混   ⑦ hūn 分 份   ⑧ hu̍t 核 佛`(~經/~-keⁿ)`  |
...
|p| ① pien 鞭`(~策/~-chheh)` 邊{边}`(無~/bô-~)`   ② pién 貶{贬} 扁`(~擔/~-tan)` 匾   ③ pièn 變{变}`(~化/~-hùe)` 遍   ⑥ piẽn 瓣 便`(~利/~-lĩ)` 卞 辨 辯{辩}   ⑧ pie̍t 蹩 別{别}`(分~/pun-~)`  |
|ph| ① phien 編{编} 扁`(~舟/~-chiu)` 篇 偏 翩   ③ phièn 片 騙{骗}  |
|m| ② mién 冕 免 勉 娩 緬{缅} 敏 憫{悯} 閔{闵}   ⑤ miên 綿{绵}  |
|t| ① tien 顛{颠}`(~簸/~-puà)` 滇 珍 巔{巅}   ② tién 碘 典 碾 腆 輾{辗} 展   ④ tiet 跌 迭 哲 秩 窒 桎 瓞 膣 蛭 蜇 輒{辄}   ⑥ tiẽn 墊{垫} 電{电} 甸 奠 澱{淀}`(沈~/sím-~)` 殿`(~後/~-ãu)`   ⑧ tie̍t 侄  |
...
|s| ① sien 仙 秈{籼}   ② sién 銑{铣} 洗 鮮{鲜} 癬{癣} 蘚{藓} 跣   ③ sièn 煽`(~風點火/~-hong-tiám-húe)` 搧`(~面龐/~-mīn-phâng)`   ④ siet 契 設{设} 楔 褻{亵}   ⑤ siên 闡{阐} 單{单}   ⑥ siẽn 單{单} 擅 膳 善 繕{缮} 腎{肾} 鱔{鳝} 撣{掸}`(~族/~-tso̍k)`  |
|j| ⑤ jiên 然 燃   ⑧ jie̍t 熱{热}  |
|k| ① kien 堅{坚} 捐 鵑{鹃} 娟   ③ kièn 見{见} 建 絹{绢}   ④ kiet 吉 潔{洁}   ⑥ kiẽn 鍵{键} 健 建 毽 犍   ⑧ kie̍t 傑{杰} 竭 桀  |
|kh| ① khien 虔   ② khién 遣 譴{谴} 犬   ④ khiet 喫{吃} 撅 契 孑 挈 黠 据   ⑤ khiên 乾{干}`(~坤/~-khun)`  |
...
|s| ① suan 喧 宣 萱 諠   ② suán 選{选}   ③ suàn 汆   ⑤ suân 旋`(~轉/~-zuán,黑~風/hek-~-hong)`   ⑥ suãn 旋`(~尿/~-jiē)`  |
|z| ② zuán 轉{转} 纂   ⑧ zua̍t 說{说} 悅{悦}  |
|k| ① kuan 關{关} 冠 觀{观} 綸{纶}   ② kuán 管 館{馆}   ③ kuàn 冠 觀{观} 罐 慣{惯} 灌 貫{贯} 眷 券 摜{掼} 盥   ④ kuat 抉 決{决} 訣{诀} 括 廓 炔 玦 刮 颳{刮}`(~風/~-hong)`   ⑥ kuãn 倦 捲{卷}`(~煎/~-chien,席~/sia̍h-~)`  |
|kh| ① khuan 寬{宽} 傾{倾} 頃{顷} 圈 髖{髋}   ② khuán 款 傾{倾} 頃{顷}   ⑤ khuân 顴{颧} 權{权}  |
|ng| ② nguán 阮 玩 頑{顽}   ⑤ nguân 顏{颜} 元 原 源   ⑥ nguãn 願{愿}`(心~/sim-~)`  |
|h| ① huan 番{畨} 翻 歡{欢} 獾   ② huán 反 返 緩{缓}   ③ huàn 販{贩} 換{换} 喚{唤} 瘓{痪} 煥{焕} 渙{涣} 幻   ④ huat 發{发}`(~現/~-hīn)` 髮{发}`(~型/~-hêng)`   ⑤ huân 樊 繁 煩{烦} 環{环} 桓 還{还} 垣   ⑥ huãn 飯{饭} 豢 宦   ⑧ hua̍t 罰{罚} 筏 伐 乏 閥{阀} 穴  |
...
|ph| ① phang 芳 蜂   ② pháng 紡{纺}   ④ phak 博 搏 伏 趴   ⑤ phâng 膀`(~胱/~-kuang)` 磅`(~礴/~-po̍h)` 傍 帆 航 龐{庞} 旁 彭 篷 彷`(彷~/phâng-~)`   ⑦ phāng 縫{缝}   ⑧ pha̍k 雹 暴 曝 瀑  |
|b| ⑧ ba̍k 墨 木  |
|m| ② máng 莽 蟒 蠓   ④ mak 衊{蔑}`(~着/~-tie̍h)`   ⑤ mâng 芒 茫 盲 氓 忙   ⑥ mãng 幕 網{网}   ⑦ māng 夢{梦}   ⑧ ma̍k 目  |
|t| ① tang 東{东} 冬 中 噹{当}`(叮~/teng-~)`   ② táng 黨{党} 等`(~待/~-thãi)` 董   ③ tàng 擋{挡}`(抵~/tí-~)` 檔{档} 凍{冻} 當{当}`(適~/sek-~)`   ④ tak 韃{鞑}   ⑤ tâng 銅{铜} 童 筒 仝 同`(相~/siang-~)`   ⑥ tãng 動{动} 筒 重 宕 蕩{荡}`(坦~/thán-~,掃~/sàu-~,遊~/iû-~)` 盪{荡}`(震~/chín-~)`   ⑦ tāng 洞   ⑧ ta̍k 達{达} 毒 獨{独} 度 值  |
...
|kh| ① khang 工 康 慷 糠 空 匡   ③ khàng 抗 亢 炕 吭   ④ khak 嘎 霍 殼{壳} 榷 確{确} 恪 麴{曲}`(鼠~草/tshṳ́-~-tshó)`  |
...
|s| ① seng 聲{声} 生 甥 牲 勝{胜} 腥 猩 惺 旌 升{陞}`(~級/~-khip,公~/kong-~)` 昇{升}`(東~/tang-~)`   ② séng 省   ③ sèng 勝{胜} 聖{圣}   ④ sek 瑟 色 識{识} 式 適{适} 釋{释} 飾{饰} 碩{硕} 昔 析 矽 晰 息 悉 夕 熄 汐 媳 植 殖 擲{掷} 晳 皙 穡{穑} 窸 蟋   ⑤ sêng 成`(~功/~-kong)` 乘`(~法/~-huap)` 誠{诚} 承`(~認/~-jīn)` 丞 榕 松`(~樹/~-chhiū)`   ⑥ sẽng 乘`(~機/~-ki)` 盛   ⑧ se̍k 熟  |
|j| ⑤ jêng 扔  |
...
|p| ④ pok 卜`(~卦/~-khùe)`   ⑤ pông 泵 乓   ⑦ pōng 磅 鎊{镑}   ⑧ po̍k 蔔{卜}`(~蔔/~-po̍k)` 僕{仆}`(奴~/nôu-~,風塵~~/hong-tîn-~-~)`  |
...
|t| ① tong 東{东} 冬 中 忠   ② tóng 董 懂   ③ tòng 棟{栋} 凍{冻} 疼 中   ④ tok 督 築{筑} 斫{斲}`(~斷/~-tuàn)`   ⑥ tõng 動{动} 重 仲   ⑧ to̍k 毒 犢{犊} 獨{独} 逐 篤{笃}  |
|th| ① thong 通   ② thóng 寵{宠} 捅 統{统}   ③ thòng 痛   ④ thok 禿{秃}   ⑤ thông 幢`(幢幡/thông-幡)` 侗 恫 洞 桐 酮 瞳 彤 童 仝 衕{同}`(衚~/hû-~)`   ⑦ thōng 洞  |
...
|h| ① hong 封 峯{峰} 鋒{锋} 風{风} 烽 轟{轰} 烘 丰`(~采/~-tshái)` 豐{丰}`(~盛/~-sẽng)` 哄`(~堂大笑/~-thâng-tuā-chhiè)`   ② hóng 蚌 俸 哄`(~騙/~-phièn)`   ③ hòng 諷{讽} 訌{讧} 哄`(~搶/~-chhiéⁿ)` 鬨{哄}`(起~/khí-~)`   ④ hok 輻{辐} 福 複{复}`(重~/têng-~,~雜/~-tsa̍p)`   ⑤ hông 逢 縫{缝} 烘 虹 鴻{鸿} 洪 宏 弘 紅{红} 肱   ⑥ hõng 奉 鳳{凤}   ⑧ ho̍k 伏 服 袱 覆`(反~/huán-~,~蓋/~-khòiⁿ)` 或 惑 域 復{复}`(~仇/~-chhiû,恢~/khue-~)`  |
...
|h| ① hiang 香 鄉{乡}   ② hiáng 晌 響{响} 享 饗{飨}   ③ hiàng 向`(方~/pang-~)` 嚮{向}`(~往/~-uáng)`   ④ hiak 蠍{蝎}  |
...
|h| ① hiong 匈 洶{汹} 凶`(~惡/~-ak,~~/~-~)` 兇{凶}`(~手/~-chhiú)`   ④ hiok 旭 毓 馥 郁`(文采~~/bûn-tshái-~-~,~達夫/~-ta̍k-hu)`   ⑤ hiông 雄  |
...
|h| ① huang 坊 芳 方 肪 封 風{风} 瘋{疯} 荒 慌 謊{谎}   ② huáng 坊 訪{访} 紡{纺} 晃 幌 恍 仿`(模~/mô-~)` 彷`(~彿/~-hut)`   ③ huàng 放   ⑤ huâng 防 妨 衡 磺 蝗 簧 皇 凰 惶 煌 徨  |
...
|t| ① tiau 碉 叼 凋 刁 貂 雕{鵰}   ③ tiàu 吊`(~燈/~-teng)` 弔{吊}`(憑~/phêng-~)`   ⑤ tiâu 條{条}   ⑥ tiãu 兆 召   ⑦ tiāu 掉 調{调}`(聲~/seng-~)`  |
...
|chh| ① chhiau 悄 鍬{锹}   ② chhiáu 嘲 悄 峭 俏 稍   ⑤ chhiâu 朝`(~代/~-thòi)` 瞧 晁  |
...
|tsh| ① tshng 艙{舱} 倉{仓}`(~庫/~-khòu)` 穿`(~耳/~-hĩⁿ)` 瘡{疮} 村   ③ tshǹg 刺`(諷~/hòng-~)`   ⑤ tshn̂g 牀{床}  |
